...
export const UserForm = ({type = 'login'}: {type : "login" | "register"}) => {
  const isLoginForm = type == 'login'
  const action = isLoginForm ? login : register 
  const initialState = isLoginForm ? loginInitialState : registerInitialState

  const [state, formAction] = useActionState(action, initialState)
  return (
    <form action={formAction} className="flex flex-col items-left gap-1">
        {isLoginForm && <div className='text-red-500'>{state.error}</div>}
        {!isLoginForm && (
        ...
        <div className="flex gap-2">
            ...
            {!isLoginForm && <div className='text-red-500'>{state.errors['email']}</div>}
        </div>
        <div className="flex gap-2">
            ...
            {!isLoginForm && <div className='text-red-500'>{state.errors['password']}</div>}
        </div>
        {!isLoginForm && (
                <div className="flex gap-2">
